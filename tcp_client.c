...
int main(int argc, char *argv[])
{
    ...
    int connect_val = connect(socket_des, (struct sockaddr *) &server_addr, sizeof(server_addr));
    if(connect_val < 0)
    {
        perror("");
        socklen_t serv_len = sizeof(server_addr);
        getpeername(socket_des, (struct sockaddr *) &server_addr, &serv_len);
        printf("Error while connecting to server %s:%d", inet_ntoa(server_addr.sin_addr), ntohs(server_addr.sin_port));
        printf("\nSocket closed\n");
        return 0;
    }
    ...
    while(strcmp(terminate,"Ctrl") != 0 && strcmp(terminate,"ctrl") != 0)
    {
        ...
        while(valid != 1)
        {
            ...
            for(i; i < data_entered[i] != '\0'; i++)
            {
                ...
                {
                    printf("- char #%d \"%c\" is invalid character.\n", i+1, data_entered[i]);
                    printf("- Enter a valid message (small letters and numbers only): ");
                    memset(data_entered, 0, sizeof(data_entered));
                    scanf("%[^\n]s",data_entered);
                    clearBuffer();
                    i = 0;
                    valid = 0;
        ...
        write(socket_des, data_sent, MAXLINE);
        printf("\n- Message:\"%s\" sent to the server.\n",data_entered);
        
        read(socket_des, &data_recv, MAXLINE);
        ...
        write(socket_des, terminate, sizeof(terminate));
